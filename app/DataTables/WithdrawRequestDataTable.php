...
class WithdrawRequestDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('wallet_balance' , function ( $query ) {
                $wallet_balance = '-';
                if($query->status == 0 ) {
                    $wallet_balance = optional($query->user) && optional($query->user)->userWallet ? optional($query->user)->userWallet->total_amount : null;
                }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('wallet_balance')->title( __('message.wallet_balance') )->searchable(false)->orderable(false),
